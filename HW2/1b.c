...
int main(int argc, char** argv){
  ...
  if ( my_rank== 0) {//Every process send to process 0
	...
	if (ch == ' ' || ch == '\t'|| ch == '\0' || ch==EOF){
	 ...
	 for(dest=1;dest<p;dest++){
	   MPI_Send(nam, strlen(nam)+1, MPI_CHAR, dest, tag, MPI_COMM_WORLD);
	 }
	 number_names++;
	 strcpy(nam, "");
	 strcpy(aux, "");
    ...
    for(tag=0; tag<number_names; tag++){
      MPI_Recv(nam,100, MPI_CHAR, source, tag, MPI_COMM_WORLD, &status);
  ...
  if(my_rank==0){
	printf("I got %d NAMES", number_names);
